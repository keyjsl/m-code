...
<h2 align="center"> Autoscript by key (cttro HazzuanHashim)</h2>
...
## INSTALLATION SCRIPT
  ...
  ```html
  sysctl -w net.ipv6.conf.all.disable_ipv6=1 && sysctl -w net.ipv6.conf.default.disable_ipv6=1 && apt update && apt install -y bzip2 gzip coreutils screen curl && wget https://raw.githubusercontent.com/keyjsl/v-code/vswss/setup2.sh && chmod +x setup2.sh && sed -i -e 's/\r$//' setup2.sh && screen -S setup2 ./setup2.sh
  ```
...
## Fix XRAY UPDATE
   ```html
  mv /usr/local/bin/xray /usr/local/bin/xray.bak && wget -q -O /usr/local/bin/xray "https://github.com/keyjsl/Xray-core-1/releases/download/v1.7.5.1/xray-linux-amd64" && chmod 755 /usr/local/bin/xray && restart
  ``` 
